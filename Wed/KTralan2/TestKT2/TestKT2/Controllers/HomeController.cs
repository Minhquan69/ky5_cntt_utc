...
namespace TestKT2.Controllers
{
    public class HomeController : Controller
    {
       ...
       QlthuVienContext db = new QlthuVienContext();
        ...
        [HttpGet]
		public JsonResult GetData(string tenloaisach)
		{
			// Truy vấn để lấy MaLoai từ tenloaisach
			var maLoai = db.TLoaiSaches
						  .Where(tls => tls.TenLoai == tenloaisach)
						  .Select(tls => tls.MaLoai)
						  .FirstOrDefault();

			if (maLoai != null)
			{
				// Sử dụng MaLoai để lấy danh sách sách
				var TenSach = (
					from ts in db.TSaches
					where ts.MaLoai == maLoai
					select ts
				).ToList();
